...
const Home = ({
  ...
  const [hasBought, setHasBought] =
    useState(false);
  const [hasLended, setHasLended] =
    useState(false);
  const [hasInspected, setHasInspected] =
    useState(false);
  ...
  const fetchDetails = async () => {
    ...
    const hasBought = await escrow.approval(
      home.id,
      buyer
    );
    ...
    const hasSold = await escrow.approval(
      home.id,
      seller
    );
    setHasSold(hasSold);
    ...
    const hasLended = await escrow.approval(
      home.id,
      lender
    );
    ...
    const hasInspected =
      await escrow.inspectionPassed(home.id);
